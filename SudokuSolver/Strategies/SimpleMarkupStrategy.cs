...
namespace SudokuSolver.Strategies
{
    class SimpleMarkupStrategy : ISudokuStrategy
    {
        ...
        public int[,] Solve(int[,] sudokuBoard)
        {
            for (int row = 0; row < sudokuBoard.GetLength(0); row++)
            {
                for (int col = 0; col < sudokuBoard.GetLength(1); col++)
                {
                    ...
                    {
                        var possibilitiesInRowAndColumn = GetPossibilitiesInRowAndCol(sudokuBoard, row, col);
                        var possibilitiesInBlock = GetPossibilitiesInBlock(sudokuBoard, row, col);
                        sudokuBoard[row, col] = GetPossibilityIntersection(possibilitiesInRowAndColumn, possibilitiesInBlock);
                    }
                }
            }
        ...
        private int GetPossibilitiesInRowAndCol(int[,] sudokuBoard, int givenRow, int givenCol)
        {
            ...
            {
                if(IsValidSingle(sudokuBoard[givenRow, givenCol]))
                {
        ...
        private int GetPossibilityIntersection(int possibilitiesInRowAndColumn, int possibilitiesInBlock)
        {
            var possibilitiesInRowAndColCharArray = possibilitiesInRowAndColumn.ToString().ToCharArray();
